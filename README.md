...
This completes the installation of VA-AHK.Integration. You should now have a folder called *VA-AHK.Integration* in your VoiceAttack *Apps* directory that houses all the files needed for the associated profile to function.
...
Comprehensive examples of the C# inline functions used to invoke the AutoHotkey functionality are given in the included VoiceAttack profile. The process basically comes down to three steps:
  ...
  ```C#
  ahk.CreateThread(""); // This is one way to create a thread
  ``` 
  ...
  ```C#
  ahk.ExecRaw("MsgBox, Hello World!"); // Other functionality is available
  ``` 
...
When you create an AutoHotkey thread it runs in the background within the VoiceAttack process instance. The thread is terminated only via the associated VA-AHK.Integration code (shown above) or by shutting down VoiceAttack. Pressing the "Stop Commands" button on VoiceAttack's UI **will NOT** terminate an active AutoHotkey thread. 
...
There are two ways to update the AutoHotkey version in VA-AHK.Integration:
  1. If new VA-AHK.Integration releases are available the *.vax* import process will automatically update your existing files in the *App* folder (assuming the package includes a newer/different version of AutoHotkey compared to what you already installed), and subsequently running any of the example commands will (if needed) automatically copy the *VA.AutoHotkey.Interop.dll* file to the required location. It is generally best to first restart VoiceAttack and then (without running any AutoHotkey-related commands that access *VA.AutoHotkey.Interop.dll*) import the updated *.vax* file.
  2. To update VA-AHK.Integration manually download the AutoHotkey_H v1 *.zip* package (found in the *Downloads* section via the link mentioned above) and take the updated *AutoHotkey.dll* files from the *Win32w* (32-bit) and *x64w* (64-bit) folders from *ahkdll* and move them into VA-AHK.Integration's *x86* (32-bit) and *x64* (64-bit) folders, respectively. It is generally best to shut down VoiceAttack before performing manual updates.
...
If you are having problems specifically with the VA-AHK.Integration commands or code then head over to the VoiceAttack User Forum, check out the *Issues* section to get an idea for how to provide enough information to request assistance, and then post to the [AutoHotkey "Integration" with VoiceAttack](http://voiceattack.com/SMF/index.php?topic=2121.0) thread within the *Inline Functions* section of the forum.

## Credits
 - Chris Mallet for the original AutoHotkey ([Source](https://autohotkey.com/foundation/history.html))
 - "Lexikos" (Steve Gray) for AutoHotkey_L (fork of AutoHotkey that is now the main branch) ([Source](https://github.com/Lexikos/AutoHotkey_L))
 - The AutoHotkey Foundation and the AutoHotkey community for the continued support and development of the current version of AutoHotkey ([Source](https://autohotkey.com/foundation/))
 - "tinku99" (inventor) and "HotKeyIt" (enhancements and continued development) for AutoHotkey_H (fork of AutoHotkey_L and provides the 32 and 64-bit *AutoHotkey.dll*) ([Source](https://github.com/HotKeyIt/ahkdll))
 - "amazing-andrew" (Andrew Smith) for AutoHotkey.Interop ([Source](https://github.com/amazing-andrew/AutoHotkey.Interop))
 - Gary for the creation and continued development of VoiceAttack (www.voiceattack.com)
 - Microsoft for [Visual Studio](https://visualstudio.microsoft.com/), which was used to develop a significant portion of this project
