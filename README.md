...
When you create an AHK thread it runs in the background within the VoiceAttack process instance. The thread is terminated only via the associated VA-AHK.Integration code ```ahk.Terminate; ``` or by shutting down VoiceAttack. Pressing the "Stop Commands" button on VoiceAttack's UI **will NOT** terminate an active AHK thread. 
